...
def add_padding_to_plaintext(plaintext, AES_length):
    ...
    '''
    if len(plaintext) % (AES_length // 8) == 0:
        return plaintext
    else:
        while len(plaintext) % (AES_length // 8) != 0:
            plaintext += " "
        return plaintext
...
def mix_columns(text, isEnxryption):
    '''
...
def AES_encrypt_helper(text, roundKeys, AES_length):
    ...
    text = add_round_key(text, roundKeys[0])
    ...
    for i in range(1, roundsMap[AES_length] + 1):
        ...
        text = add_round_key(text, roundKeys[i])
...
def AES_encrypt(plaintext, keytext, AES_length):
    ...
    roundKeys = key_generation(key)

    # Encrypt the plaintext
    blocksize = AES_length // 8
    ciphertext = []
...
def AES_decrypt_helper(ciphertext, roundKeys, AES_length):
    ...
    for i in range(roundsMap[AES_length], 0, -1):
        ...
        ciphertext = add_round_key(ciphertext, roundKeys[i])
    ...
    # Round 0 : Add round key 0
    ciphertext = add_round_key(ciphertext, roundKeys[0])
...
def main():
    ...
    ciphertext = AES_encrypt("Never Gonna Give you up.", "BUET CSE19 Batch", 128)
    print(ciphertext)
    plaintext = AES_decrypt(ciphertext, "BUET CSE19 Batch", 128)
    print(plaintext)

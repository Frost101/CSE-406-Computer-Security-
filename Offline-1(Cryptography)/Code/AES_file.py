...
def AES_encrypt_file(fileArray, keytext, AES_length, IV):
    '''
    ...
    # Add padding to the fileContent if needed
    file = generalized_Padding(fileArray)
    ...
    for i in range(0, len(file), blocksize):
        text_to_send = file[i:i + blocksize]
...
def AES_decrypt_file(ciphertext, keytext, AES_length, IV):
    '''
...
def convert_to_1D_hexarray_file(file_contents):
    hex_array = []
    for i in range(0, len(file_contents)):
        hex_array.append((hex(file_contents[i])[2:].upper()))
    return hex_array


def file_output(hexArray, output_file):
    temp = []
    for i in range(0, len(hexArray)):
        temp.append(BitVector(hexstring=hexArray[i]).int_val())
...
def main():
    ...
    hex_array = convert_to_1D_hexarray_file(file_contents)

    cipher = AES_encrypt_file(hex_array, key, AES_length, IV)
    # print(cipher)

    decryptedData = AES_decrypt_file(cipher, key, AES_length, IV)
    # print(decryptedData)

    decryptedData = generalized_unPadding(decryptedData)

    file_output( decryptedData, output_file)
